...
export function walkthrough02_Embedding(args: IWalkthroughArgs) {
    ...
    commentary(wt)`
之前我们看到 token 是如何通过一个简单的查表被映射为整数序列的。
这些整数，即 ${c_blockRef('_token 索引_', state.layout.idxObj, DimStyle.TokenIdx)}，是我们在这个模型中唯一一次看到整数。
从那之后，我们将全部使用浮点数（小数）。

让我们来看看第 4 个 token（索引 3）是如何被用来生成 ${c_blockRef('_输入嵌入_', state.layout.residual0)} 的第 4 列向量的。`;
    breakAfter();
    ...
    commentary(wt)`
我们使用 token 索引（在这个例子中 ${c_str('B', DimStyle.Token)} = ${c_dimRef('1', DimStyle.TokenIdx)})来选取左侧 ${c_blockRef('_token 嵌入矩阵_', state.layout.tokEmbedObj)} 的第 2 列。
注意这里我们使用的是从 0 开始的索引，所以第一列的索引是 0。

这样就产生了一个大小为 ${c_dimRef('_C_ = 48', DimStyle.C)} 的列向量，我们称之为 token 嵌入。
    `;
    breakAfter();
    ...
    commentary(wt)`
由于我们正在看第 4 个 _位置_（t = ${c_dimRef('3', DimStyle.T)})的 token ${c_str('B', DimStyle.T)}，我们还需要取 ${c_blockRef('_位置嵌入矩阵_', state.layout.posEmbedObj)} 的第 4 列。

这同样产生了一个大小为 ${c_dimRef('_C_ = 48', DimStyle.C)} 的列向量，我们称之为位置嵌入。
    `;
    breakAfter();
    ...
    commentary(wt)`
注意，这两组位置嵌入和 token 嵌入都是在训练过程中学习得到的（由它们的蓝色标记可知）。

现在我们有了这两个列向量，我们只需将它们相加，就能得到另一个大小为 ${c_dimRef('_C_ = 48', DimStyle.C)} 的列向量。
`;
    ...
    commentary(wt)`
现在我们对输入序列中的所有 token 运行相同的过程，创建一组同时包含 token 值和位置信息的向量。
...
`;
    ...
    commentary(wt)`
你可以将鼠标悬停在 ${c_blockRef('_输入嵌入_', state.layout.residual0)} 矩阵的单个格子上，查看其计算过程和来源。

我们看到，对序列中的所有 token 运行此过程，会得到一个大小为 ${c_dimRef('_T_', DimStyle.T)} × ${c_dimRef('_C_', DimStyle.C)} 的矩阵。
${c_dimRef('_T_', DimStyle.T)} 代表 _时间_（time），即你可以把序列中靠后的 token 想象为时间上靠后的 token。
${c_dimRef('_C_', DimStyle.C)} 代表 _通道_（channel），但也常被称为"特征"、"维度"或"嵌入大小"。这个长度 ${c_dimRef('_C_', DimStyle.C)}
是模型的几个"超参数"之一，由设计者根据模型大小和性能之间的权衡来选择。

这个矩阵，我们称之为 ${c_blockRef('_输入嵌入_', state.layout.residual0)}，现在准备好向下传递穿过模型了。
这组 ${c_dimRef('T', DimStyle.T)} 列，每列长度为 ${c_dimRef('C', DimStyle.C)}，将成为本指南中常见的画面。
`;
