...
上一节得到的 ${c_blockRef('_输入嵌入_', state.layout.residual0)} 矩阵是我们第一个 Transformer 块的输入。

Transformer 块的第一步是对这个矩阵应用 _层归一化_（layer normalization）。这是一个
对矩阵每一列分别进行归一化操作的步骤。`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
归一化是深度神经网络训练中的重要步骤，它有助于提高模型在训练过程中的稳定性。

我们可以单独看待每一列，所以让我们先聚焦于第 4 列(${c_dimRef('t = 3', DimStyle.T)})。`;
    ...
    breakAfter();
    commentary(wt)`
目标是使该列的平均值等于 0，标准差等于 1。为此，我们首先计算该列的这两个量(${c_blockRef('均值 (\u03bc)', ln.lnAgg1)} 和 ${c_blockRef('标准差 (\u03c3)', ln.lnAgg2)})，然后减去均值并除以标准差。`;
    ...
    breakAfter();
    commentary(wt)`
这里我们使用的符号是 E[x] 表示（长度为 ${c_dimRef('C', DimStyle.C)} 的列的）平均值，Var[x] 表示方差。方差就是标准差的平方。epsilon 项（ε = ${<>1&times;10<sup>-5</sup></>})是为了防止除以零。

我们在聚合层中计算并存储这些值，因为我们要将它们应用于该列中的所有值。

最后，一旦我们得到归一化后的值，我们会将列中的每个元素乘以一个学习得到的 ${c_blockRef('权重 (γ)', ln.lnSigma)}，再加上一个 ${c_blockRef('偏置 (β)', ln.lnMu)}，最终得到 ${c_blockRef('归一化后的值', ln.lnResid)}。`;
    ...
    breakAfter();
    commentary(wt)`
我们对 ${c_blockRef('输入嵌入矩阵', layout.residual0)} 的每一列运行这个归一化操作，得到的结果就是
${c_blockRef('归一化后的输入嵌入', ln.lnResid)}，它准备好进入自注意力层了。
`;
